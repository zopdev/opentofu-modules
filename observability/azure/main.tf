locals {
  ...
  app_namespaces = {
    ...
    openobserve = local.enable_openobserve ? {
      services = [for instance in var.openobserve : "${instance.name}:5080" if instance.enable && instance.enable_ingress != false]
      ingress  = anytrue([for instance in var.openobserve : instance.enable && try(instance.enable_ingress, true)])
    } : null
  }
...
resource "kubernetes_ingress_v1" "service_ingress" {
  for_each = {for service, value in local.services_list : service => value if var.domain_name != ""}
  metadata {
