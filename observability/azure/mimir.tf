resource "azurerm_storage_container" "mimir_container" {
  ...
  name                  = "${local.cluster_name}-mimir-container-${var.observability_suffix}"
...
resource "kubernetes_secret" "mimir-azure-credentials" {
  count   = local.enable_mimir ? 1 : 0
  metadata {
    name        = "${local.cluster_name}-mimir-azure-credentials"
    namespace   = "mimir"
    labels      = { app = var.app_name }
    annotations = {
      "kubernetes.io/service-account.name" = "${local.cluster_name}-mimir-azure-credentials"
    }
  }
...
locals {
  mimir_values = local.enable_mimir ? templatefile("${path.module}/templates/mimir-values.yaml", {
    # Storage config
    CONTAINER      = azurerm_storage_container.mimir_container[0].name
    ...
    store_gateway_persistence_volume_size       = try(var.mimir.store_gateway.persistence_volume.size, "50Gi")
