...
locals{
  # this is namespace level alerts:
  ...
  prom_template = local.prometheus_enable ? templatefile(
    ...
    {
      PROMETHEUS_DISK_SIZE          = try(
          var.observability_config.prometheus.persistence.disk_size != null
          ? var.observability_config.prometheus.persistence.disk_size
          : "50Gi",
        "50Gi"
      )
      PROMETHEUS_RETENTION_SIZE     = try(
          var.observability_config.prometheus.persistence.retention_size != null
          ? var.observability_config.prometheus.persistence.retention_size
          : "45GB",
        "45GB"
      )
      PROMETHEUS_RETENTION_DURATION = try(
          var.observability_config.prometheus.persistence.retention_duration != null
          ? var.observability_config.prometheus.persistence.retention_duration
          : "10d",
        "10d"
      )
...
resource "kubectl_manifest" "cluster-alerts" {
  count      = local.prometheus_enable ? 1 : 0
  yaml_body  = local.cluster_alert_config
  depends_on = [helm_release.prometheus]
}
