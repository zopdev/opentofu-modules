...
module "service_deployment" {
  ...
  app_secrets     = each.value.db_name != null || each.value.custom_secrets != null || each.value.redis == true || each.value.redis_configs != null ? ["${each.key}-application-secrets"] : []
  ...
  db_ssl_enabled   = false ###### Need to change this 
