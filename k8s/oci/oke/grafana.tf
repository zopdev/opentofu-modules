locals {
  ...
  grafana_values = local.grafana_enable ? templatefile("${path.module}/templates/grafana-values.yaml", {
    NAMESPACE                         = "monitoring"
    GRAFANA_HOST                      = local.grafana_host
    GRAFANA_ENABLED                   = local.grafana_enable
    GRAFANA_TLS_HOST                  = "*.${local.domain_name}"
    GRAFANA_OBS_ADMIN_PASSWORD        = try(random_password.observability_admin[0].result, "")
    ...
    PERSISTENCE_DISK_SIZE             = try(var.observability_config.grafana.persistence.disk_size, "10Gi")
    GRAFANA_DB_NAME                   = try(var.observability_config.grafana.persistence.type == "db" ? "grafana" : "", "")
    GRAFANA_DB_TYPE                   = try(var.observability_config.grafana.persistence.type == "db" ? "postgres" : "", "")
    GRAFANA_DB_HOST                   = try(var.observability_config.grafana.persistence.type == "db" ? module.grafana_db[0].db_url : "", "")
    GRAFANA_DB_PASSWORD               = try(var.observability_config.grafana.persistence.type == "db" ? module.grafana_db[0].db_password : "", "")
    GRAFANA_DB_USER                   = try(var.observability_config.grafana.persistence.type == "db" ? module.grafana_db[0].db_admin_user : "", "")
    GRAFANA_MIN_REPLICA               = try(var.observability_config.grafana.min_replica, 1)
    GRAFANA_MAX_REPLICA               = try(var.observability_config.grafana.max_replica, 10)
    GRAFANA_REQUEST_MEMORY            = try(var.observability_config.grafana.request_memory, "100Mi")
    GRAFANA_REQUEST_CPU               = try(var.observability_config.grafana.request_cpu, "100m")
    GRAFANA_LIMIT_MEMORY              = try(var.observability_config.grafana.limit_memory, "500Mi")
    GRAFANA_LIMIT_CPU                 = try(var.observability_config.grafana.limit_cpu, "500m")
    GRAFANA_DASHBOARD_LIMIT_MEMORY    = try(var.observability_config.grafana.dashboard.limit_memory, "512Mi")
    GRAFANA_DASHBOARD_LIMIT_CPU       = try(var.observability_config.grafana.dashboard.limit_cpu, "512m")
    GRAFANA_DASHBOARD_REQUEST_MEMORY  = try(var.observability_config.grafana.dashboard.request_memory, "256Mi")
    GRAFANA_DASHBOARD_REQUEST_CPU     = try(var.observability_config.grafana.dashboard.request_cpu, "256m")
    GRAFANA_DATASOURCE_LIMIT_MEMORY   = try(var.observability_config.grafana.datasource.limit_memory, "512Mi")
    GRAFANA_DATASOURCE_LIMIT_CPU      = try(var.observability_config.grafana.datasource.limit_cpu, "512m")
    GRAFANA_DATASOURCE_REQUEST_MEMORY = try(var.observability_config.grafana.datasource.request_memory, "256Mi")
    GRAFANA_DATASOURCE_REQUEST_CPU    = try(var.observability_config.grafana.datasource.request_cpu, "256m")
    ENABLE_SSO                        = try(var.observability_config.grafana.configs.enable_sso, false)
    ALLOWED_DOMAINS                   = local.grafana_allowed_domains
    # Uncomment these when you have OCI secrets configured
    # OAUTH_ID                        = try(data.oci_secrets_secretbundle.oauth_client_id[0].secret_bundle_content[0].content, null)
    # OAUTH_SECRET                    = try(data.oci_secrets_secretbundle.oauth_client_secret[0].secret_bundle_content[0].content, null)
  }) : null
}
...
resource "helm_release" "grafana" {
  ...
  values = [
    local.grafana_values[count.index]
  ]
