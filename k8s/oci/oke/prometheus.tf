...
locals{
  ...
  prom_values = local.prometheus_enable ? templatefile("${path.module}/templates/prometheus-values.yaml", {
    PROMETHEUS_DISK_SIZE          = try(var.observability_config.prometheus.persistence.disk_size, "50Gi")
    PROMETHEUS_RETENTION_SIZE     = try(var.observability_config.prometheus.persistence.retention_size, "20GB")
    PROMETHEUS_RETENTION_DURATION = try(var.observability_config.prometheus.persistence.retention_duration, "7d")
    CLUSTER_NAME                  = local.cluster_name
    REMOTE_WRITE_CONFIGS          = jsonencode(local.remote_write_config)

    ALERTS_ENABLED = (
    jsonencode(local.cluster_moogsoft_alerts) != "" ||
    jsonencode(local.namespace_teams_webhook) != "" ||
    jsonencode(local.cluster_teams_alerts) != "" ||
    jsonencode(local.google_chat_alerts) != "" ||
    jsonencode(local.cluster_slack_alerts) != "" ||
    jsonencode(local.cluster_webhook_alerts) != ""
    ) ? true : false

    MOOGSOFT_ALERTS_ENABLED = local.cluster_moogsoft_alerts != {} ? true : false
    MS_TEAMS_ALERT_ENABLED  = (jsonencode(local.namespace_teams_webhook) != "" || jsonencode(local.cluster_teams_alerts) != "") ? true : false
    ...
    GOOGLE_CHAT_ALERTS_ENABLED = local.google_chat_alerts != "" ? true : false
    SLACK_CHAT_ALERTS_ENABLED  = local.cluster_slack_alerts != "" ? true : false
    WEBHOOK_ALERTS_ENABLED     = local.cluster_webhook_alerts != "" ? true : false
    ...
    PAGER_DUTY_ALERTS_ENABLED = local.cluster_pagerduty_alerts != "" ? true : false
  ...
  cluster_alerts = templatefile(
    "./templates/cluster-level-alerts.yaml",
    {
      cluster_memory_usage_request_underutilisation_threshold = var.cluster_alert_thresholds == null ? 20 : try(var.cluster_alert_thresholds.memory_underutilisation, 20)
      cluster_cpu_usage_request_underutilisation_threshold    = var.cluster_alert_thresholds == null ? 20 : try(var.cluster_alert_thresholds.cpu_underutilisation, 20)
      cluster_node_count_max_value                             = var.node_config.size
      cluster_node_count_threshold                             = var.cluster_alert_thresholds == null ? 80 : try(var.cluster_alert_thresholds.node_count, 80)
      cluster_pod_count_threshold                               = var.cluster_alert_thresholds == null ? 80 : try(var.cluster_alert_thresholds.pod_count, 80)
      cluster_total_cpu_utilization_threshold                  = var.cluster_alert_thresholds == null ? 80 : try(var.cluster_alert_thresholds.cpu_utilisation, 80)
      cluster_total_memory_utilization_threshold               = var.cluster_alert_thresholds == null ? 20 : try(var.cluster_alert_thresholds.memory_utilisation, 20)
      cluster_disk_utilization_threshold                        = var.cluster_alert_thresholds == null ? 80 : try(var.cluster_alert_thresholds.disk_utilization, 80)
      cluster_name                                             = local.cluster_name
      cortex_enabled                                           = try(var.observability_config.cortex.enable, false)
      nginx_5xx_percentage_threshold                            = var.cluster_alert_thresholds == null ? 5 : try(var.cluster_alert_thresholds.nginx_5xx_percentage_threshold, 5)
      cortex_disk_utilization_threshold                         = var.cluster_alert_thresholds == null ? 80 : try(var.cluster_alert_thresholds.cortex_disk_utilization_threshold, 80)
      prometheus_disk_utilization_threshold                     = var.cluster_alert_thresholds == null ? 80 : try(var.cluster_alert_thresholds.prometheus_disk_utilization_threshold, 80)
    }
  )
}
...
resource "kubectl_manifest" "cluster-alerts" {
  count      = local.prometheus_enable ? 1 : 0
  yaml_body  = local.cluster_alerts
