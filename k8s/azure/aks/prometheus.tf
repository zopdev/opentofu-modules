...
locals{
  ...
  cluster_alerts_config = templatefile(
...
resource "helm_release" "alerts_teams" {
  count = jsonencode(local.namespace_teams_webhook) == "" && jsonencode(local.cluster_teams_alerts) == "" ? 0 : 1
...
resource "kubectl_manifest" "cluster-alerts" {
  ...
  yaml_body = local.cluster_alerts_config
  depends_on = [helm_release.prometheus]
}
...
resource "azurerm_key_vault_secret" "observability_admin_secret" {
  count        = local.grafana_enable ? 1 : 0
  name         = "${local.cluster_name}-grafana-admin-secret"
  value        = random_password.observability_admin[0].result
