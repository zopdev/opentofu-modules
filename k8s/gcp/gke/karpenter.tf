...
locals {
  nodeclass_yaml = var.karp_enable ? templatefile("./templates/karpenter-gcp-nodeclass.yaml", {
    SERVICE_ACCOUNT = google_service_account.karpenter.email
    ENVIRONMENT = var.app_env
  }) : null

  nodepool_yaml = var.karp_enable ? templatefile("./templates/karpenter-gcp-nodepool.yaml", {
