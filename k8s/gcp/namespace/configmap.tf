  ...
  data = merge(
    {
      ...
      "DB_REPLICA_HOST" = try(each.value.datastore_configs.read_replica, false) == true ? "${each.value.datastore_configs.name}-sql-replica.db" : null
      "DB_REPLICA_PORT" = try(each.value.datastore_configs.read_replica, false) == true ? module.sql_db_v2[each.value.datastore_configs.name].db_port : null
