  ...
  data = merge(
    {
      ...
      "DB_USER"         = each.value.db_name != null ? module.sql_db[0].db_user["${var.namespace}-${each.value.db_name}"] : each.value.datastore_configs != null ? module.sql_db_v2[each.value.datastore_configs.name].db_user["${var.namespace}-${each.value.datastore_configs.databse}"] : null
      "DB_DIALECT"      = each.value.db_name != null ? module.sql_db[0].db_type :  each.value.datastore_configs != null ?  module.sql_db_v2[each.value.datastore_configs.name].db_type : null
      ...
      "DB_PORT"         = each.value.db_name != null ? module.sql_db[0].db_port : each.value.datastore_configs != null ?  module.sql_db_v2[each.value.datastore_configs.name].db_port : null
      "DB_REPLICA_HOST" = each.value.db_name != null ? (try(var.sql_db.read_replica, false) ? "${var.namespace}-sql-replica.db" : null) : each.value.datastore_configs != null ? (try(tobool(var.sql_list[each.value.datastore_configs.name].read_replica), false) ? "${each.value.datastore_configs.name}-sql-replica.db" : null) : null
      "DB_REPLICA_PORT" = each.value.db_name != null ? (try(var.sql_db.read_replica, false) ? module.sql_db[0].db_port : null) : each.value.datastore_configs != null ? (try(tobool(var.sql_list[each.value.datastore_configs.name].read_replica), false) ? module.sql_db_v2[each.value.datastore_configs.name].db_port : null) : null
