  ...
  data = merge(
    {
      ...
      "DB_REPLICA_HOST" = try(each.value.datastore_configs.read_replica, false) ? "${each.value.datastore_configs.name}-sql-replica.db" : null
      "DB_REPLICA_HOST" = try(each.value.datastore_configs.read_replica, false) ? module.sql_db_v2[each.value.datastore_configs.name].db_port : null
  ...
  data = merge(
    {
      ...
      "DB_PORT"         = each.value.db_name != null ? module.sql_db[0].db_port : each.value.datastore_configs != null ?  module.sql_db_v2[each.value.datastore_configs.name].db_port : null
      "DB_REPLICA_HOST" = try(each.value.datastore_configs.read_replica, false) ? "${each.value.datastore_configs.name}-sql-replica.db" : null
      "DB_REPLICA_PORT" = try(each.value.datastore_configs.read_replica, false) ? module.sql_db_v2[each.value.datastore_configs.name].db_port : null
      "REDIS_HOST"      = each.value.redis == true || each.value.local_redis == true ? (each.value.redis == true ? "${var.namespace}-redis" : "redis-master-master") : (each.value.redis_configs != null ? "${each.value.redis_configs.name}-${var.namespace}-redis" : null),
      "REDIS_PORT"      = each.value.redis == true || each.value.local_redis == true ? "6379" : (each.value.redis_configs != null ? each.value.redis_configs.port : null)
    })
}
