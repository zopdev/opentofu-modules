  ...
  data = merge(
    {
      ...
      "DB_USER"         = lookup(local.db_map, "${var.namespace}-${lookup(local.sql_config_list, each.key, {}).db_name}", null) != null ? lookup(local.db_map, "${var.namespace}-${lookup(local.sql_config_list, each.key).db_name}").user : (each.value.db_name != null ? module.sql_db[0].db_user["${var.namespace}-${each.value.db_name}"] : null)
      "DB_DIALECT"      = lookup(local.sql_config_list, each.key, null) != null ? lookup(local.sql_config_list, each.key).type : (each.value.db_name != null ? module.sql_db[0].db_type : null)
